...
from .mcocTools import (StaticGameData, PagesMenu, KABAM_ICON, COLLECTOR_ICON, CDTHelperFunctions, GSHandler, GSExport)
...
class Hook:
    ...
    async def _roster_stats(self, ctx, user: discord.User = None):
        ...
        for star in (6, 5, 4, 3, 2, 1):
            ...
            collected = round(count / available * 100, 4)
            data.add_field(name='\n{}★ Champion Count: {}'.format(star, count),
                           value='\nPercent of Roster: {}%\nCollection Index: {} of {} = {}'
                           .format(round(count/total*100, 4), count, available, collected))
        pages.append(data)
        for star in (6, 5, 4, 3, 2, 1):
            ...
            for klass in ('Cosmic', 'Tech', 'Mutant', 'Skill', 'Science', 'Mystic'):
                ...
                if stats[klass][star]['count'] > 0:
                    ...
                    collected = round(count/available*100, 4)
                    list.append('{0} Count: {1}\n{0} Power: {2:,}\n{3}% of Roster\nCollection Index {4} of {5} = {6}%'
                                .format(klass, count, power, percent, count, available, collected))
                    if len(list) > 0:
